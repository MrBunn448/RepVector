...
The project maintains a dedicated test suite within the `WorkoutTracker.Logic.Tests` project.

### Tools
* **xUnit**: Used as the primary testing engine for its modern approach to test discovery and parallel execution.
* **Moq**: Utilized for creating mock objects of repository interfaces. This allows for isolated unit testing of business logic without requiring a live database connection.
* **Coverlet**: Integrated for measuring code coverage to ensure critical business rules are thoroughly verified.

### Integration Testing
The `WorkoutTracker.IntegrationTests` project provides end-to-end verification of the API endpoints using `Microsoft.AspNetCore.Mvc.Testing`. These tests host the application in-memory to validate the entire request lifecycle, including routing, authentication headers, and service orchestration.

### Rationale
* **Isolation**: Mocking the DAL ensures that tests are fast and deterministic.
* **Maintainability**: Clear separation between logic and data access allows for testing complex authorization rules and edge cases (e.g., unauthorized access attempts) in a controlled environment.
* **End-to-End Reliability**: Integration tests ensure that the API and Logic layers are correctly wired together.
